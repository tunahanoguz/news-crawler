...
class ParsedArticleService:
    ...
    def create(article: Article):
        with DbContextManager() as db:
            query = """
                INSERT INTO parsed_articles (title, description, content, tags, link, category, company, published_date)
    ...
    def delete(article_id: int):
        with DbContextManager() as db:
            query = """DELETE FROM parsed_articles WHERE id = %s"""
            cur = db.cursor()
            cur.execute(query, (article_id,))
    ...
    def delete_all():
        with DbContextManager() as db:
            query = """TRUNCATE TABLE parsed_articles"""
            cur = db.cursor()
            cur.execute(query)
            cur.close()
